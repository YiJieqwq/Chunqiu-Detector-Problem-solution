...
> 文档基线：4.5.5(68) ｜ 最后更新：2026-09-25
> 本次新增：`TEESimulator detector` 的 4.5.6(69) 静态分析说明；其他条目未因此升级为 4.5.6 已复核结论。
...
> 适用版本：**春秋 4.5.6(69)**；异常编号：**296**。本条来自主 APK 的静态分析，尚未进行真机验证；标题保留应用原拼写。

#### 检测方式

这是一项 **Android Keystore 的 AES-GCM 配对计时检查**，不是扫描是否安装了某个同名模块，也不是单凭证书链判断。

1. 在 `AndroidKeyStore` 中生成 AES 密钥，使用 `AES/GCM/NoPadding` 构造可正常解密的密文；再复制密文并篡改一个字节，保持长度、密钥、IV 和 AAD 不变，作为认证失败对照。
2. 两条路径各预热一次，再进行 **8 组配对测量**，按轮次奇偶交替先后顺序。
3. 计时覆盖 `Cipher.getInstance()`、初始化、AAD 更新、`doFinal()` 及成功或异常处理，**不是只测 TEE 内部算法耗时**。
4. 先校验行为：合法密文应解密成功，篡改密文应以认可的认证失败形式结束。若不符合，走 `UNAVAILABLE`（不可用）路径，而不是直接认定计时命中。

静态重建的命中条件须同时满足：

- 行为校验不匹配数为 `0`；
- 8 组中至少 **6 组正常路径比认证失败路径更慢**；
- 配对耗时差的上中位数 `D > max(750000 ns, 3 × MAD)`，严格大于；
- 两类耗时的上中位数满足 `100 × S >= 125 × F`，即正常路径至少为认证失败路径的 **1.25 倍**。

其中，`S`、`F` 分别为正常 / 认证失败路径耗时的上中位数；`D` 为每组“正常耗时减认证失败耗时”的上中位数；`MAD` 为这些配对差值相对 `D` 的绝对偏差的上中位数。8 个数排序后取**第 5 个**，不取中间两项平均；`D` 也不等于简单地用 `S - F` 替代。`750000 ns = 0.75 ms` 是**差值下限**，不是异常解密的绝对耗时上限。

状态映射为 `1 = PASS`、`2 = HIT`、`3 = UNAVAILABLE`；只有状态 `2` 才添加异常编号 `296`。

#### 解决办法

**目前没有经本项目真机验证的通用解决办法。** 建议先记录检测器版本、系统构建、相关模块及配置，在相同环境下重复扫描，区分稳定命中与偶发变化；如需对照排查，每次只改变一个已知可恢复的配置变量。这些步骤用于定位，不保证通过。

不要把更换 `keybox`、隐藏应用包名或任意更换密钥模块当作本条已证实有效的处理方式；本条证据尚不能建立这样的因果关系。

#### 备注

- **已确认的静态事实**包括 AES-GCM 对照构造、8 组采样、比较常量与结构，以及状态 `2` 到编号 `296` 的映射；**尚未完全闭合**的是部分混淆包装条件的运行时可达性。未将辅助 APK 的独立检查当作主 APK 的同一机制。
- 请求 `AndroidKeyStore` **不等于已证明每台设备都由硬件 TEE 执行**。设备 / KeyMint / 加密提供者的兼容性、实际耗时分布及误报率仍待实测；**命中不能单独证明安装了 TEESimulator，也不能证明设备是模拟器**。
- `error.txt` 是不可用 / 错误记录的报告，**不是完整的 PASS / HIT 计时日志**，且可能保留旧文件。不能凭其存在或缺失推断本轮是否命中；异常卡片也不能假定会展示全部中位数 / MAD 诊断字段。
- 本条与 `TEE 伪造(2)` 的混合用途密钥检查、`无效的伪造信息(1)` 的 Widevine 检查分开理解，不共用判据或解决办法；本次新增不代表那些旧条目已完成 4.5.6 复核。
